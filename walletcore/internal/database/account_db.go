...
func (a *AccountDB) FindById(id string) (*entity.Account, error) {
	...
	stmt, err := a.DB.Prepare(
		"SELECT a.id, a.client_id, a.balance, a.created_at, c.id, c.name, c.email, c.created_at FROM accounts a JOIN clients c ON a.client_id = c.id WHERE a.id = ?",
	)
	if err != nil {
		return nil, err
	}
	...
	err = row.Scan(&account.ID,
		...
		&account.CreatedAt,
		&account.Client.ID,
		&account.Client.Name, &account.Client.Email, &account.Client.CreatedAt)
	if err != nil {
		return nil, err
	}
